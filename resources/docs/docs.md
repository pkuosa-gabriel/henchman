...
The `home-routes` handler in the `henchman.routes.home` namespace
...
The `home-page` function will in turn call the `henchman.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `henchman.handler` namespace:
...
Request middleware functions are located under the `henchman.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `henchman.dev-middleware` namespace found in
